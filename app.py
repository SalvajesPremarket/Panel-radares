...
[18/9/2026 10:29] Minor Gomez: with c3:
...
with d1:
    opciones_cruce = ["Hacia arriba", "Hacia abajo"]
    DIRECCION_CRUCE = st.selectbox(
        "Cruce EMA20",
        opciones_cruce,
        index=opciones_cruce.index(cfg["direccion_cruce"]) if cfg["direccion_cruce"] in opciones_cruce else 0,
    )
with d2:
    opciones_macd = ["Positivo", "Negativo"]
    MACD_SIGNO = st.selectbox(
        "MACD",
        opciones_macd,
        index=opciones_macd.index(cfg["macd_signo"]) if cfg["macd_signo"] in opciones_macd else 0,
    )
with d3:
    opciones_top_n = [5, 10, 15, 20, 25, 30]
    TOP_N = st.selectbox(
        "Mostrar Top",
        opciones_top_n,
        index=opciones_top_n.index(cfg["top_n"]) if cfg["top_n"] in opciones_top_n else 1,
    )
st.markdown('</div>', unsafe_allow_html=True)
